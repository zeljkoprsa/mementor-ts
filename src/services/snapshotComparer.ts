...
interface SnapshotDiff {
  ...
  metrics: {
    old: HealthMetrics;
    new: HealthMetrics;
    changes: Partial<Record<keyof HealthMetrics, {
...
export class SnapshotComparer {
  private async parseSnapshot(filePath: string): Promise<{ metadata: SnapshotMetadata; content: string[] }> {
    ...
    return {
      metadata: { version, created_at: created, updated_at: updated, health_metrics: healthMetrics },
  ...
  private compareMetrics(old: HealthMetrics, current: HealthMetrics): SnapshotDiff['metrics']['changes'] {
  ...
  formatDiff(diff: SnapshotDiff): string {
    ...
    for (const [key, change] of Object.entries(diff.metrics.changes)) {
      ...
      if (typeof change.old === 'number' && typeof change.new === 'number') {
        ...
        lines.push(`  ${change.old} → ${change.new} ${trend} (${change.delta! >= 0 ? '+' : ''}${change.delta})`);
      } else {
